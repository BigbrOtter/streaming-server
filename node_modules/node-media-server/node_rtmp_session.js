...
var remove = 0;
var StreamChecker = require('./streamChecker.js');
var streamChecker;
var frameCounter = 0;
...
class NodeRtmpSession {
  constructor(config, socket) {
    ...
    this.streamChecker = new StreamChecker(this);
  ...
  onSocketData(data) {
    /*
    Logger.log("SocketData");
    //Logger.log(JSON.stringify(data));
    if(data.includes("digi")){
      Logger.log("Digi found");
      var counter =0;
      var payloadstart =0;
      //find begin payload
      for(payloadstart = 0;payloadstart < data.length; payloadstart++){
          if(data[payloadstart] == 2){
            if(counter ==0){
              counter++;
            }
            else{
      ...
      payloadstart = payloadstart -counter;
      Logger.log(JSON.stringify(data));

      if(data[data.length-66] === 195){
        //remove = - 71;
        remove = -74;
        Logger.log('195 found');
      }
      else{
        //remove = -70;
        remove = -73;
      ...
      //copy hash to hashbuffer from data 
      //var hashBuffer = data.slice(data.length-63, data.length);
      var hashBuffer = data.slice(data.length-64, data.length);
      //remove digisign + hash from data
      data = data.slice(0, remove );
      //data check
      Logger.log("hashBuffer");
      Logger.log(JSON.stringify(hashBuffer));
      Logger.log("Data sliced");
      Logger.log(JSON.stringify(data));

      //get payload from data
      Logger.log(`payloadstart ${payloadstart} data length ${data.length}`);
      var hashdata = data.slice(payloadstart, data.length);
      Logger.log("hashdata");
      Logger.log(JSON.stringify(hashdata));

      //payload bytes to string
      var hashDataString = '';
      for(var i=0; i<hashdata.length; i++){
        if(hashdata[i] != 195){
          hashDataString +=hashdata[i];
        }  
      }
      Logger.log("hashdata string");
      Logger.log(hashDataString);
      
      var hashCheck = sha256(hashDataString);
      hashCheck = hashCheck.substr(1);
      var hash = hashBuffer.toString();
      hash = hash.substr(1);
      //hash check
      if(hash == hashCheck){
        Logger.log('hash is goed ');
      }
      else{
        Logger.log("hash is fout:"+ hashCheck+" en "+ hash);
      }
    }
    */
    ...
    while (offset < bytes) {
      switch (this.parserState) {
        ...
        case RTMP_PARSE_PAYLOAD:
          ...
          offset += size;
          //Logger.log(this.parserPacket.header.length);

          //Logger.log(this.parserPacket.bytes);
          
         // Logger.log(`parserpacket bytes ${this.parserPacket.bytes}`);
          //Logger.log(`parserpacker header length ${this.parserPacket.header.length}`);
  ...
  rtmpChunkMessageHeaderRead() {
    ...
    if (this.parserPacket.header.fmt <= RTMP_CHUNK_TYPE_1) {
     // Logger.log(`parserbuffer`);
      //Logger.log(JSON.stringify(this.parserBuffer));
      //Logger.log(`offset ${offset}`);
      //Logger.log(`parserbuffer readunitbe ${this.parserBuffer.readUIntBE(offset, 3)}`);
      //18 323
  ...
  rtmpDataHandler() {
    ...
    if(this.frameCounter == 10){
      ...
      this.streamChecker(this.publishStreamId, this.parserPacket.header.timestamp, hashDataString);
      this.frameCounter = 0;
    }
    else{
      this.frameCounter++;
