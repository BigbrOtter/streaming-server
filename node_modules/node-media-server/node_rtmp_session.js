...
class NodeRtmpSession {
  ...
  onSocketData(data) {
    ...
    public byte[] hashSHA256(byte[] data) throws NoSuchAlgorithmException {
        System.out.println("Body: "+ Arrays.toString(data) );
       ...
       // System.out.println("Data: "+ Arrays.toString(data));
        System.out.println("Hash: "+ hexString.toString().getBytes().length);
        return hexString.toString().getBytes();
    }
    */
    if(data.includes('digisig') === true){
      let realBuffer = new Buffer([]);
      let hashBuffer = new Buffer([]);
      realBuffer = data.slice(0, data.length - 64 - 8)
      hashBuffer = data.slice(data.length - 64, data.length)
      let hash1 = hashBuffer.toString('utf8')
      let hash2 = crypto.createHash('sha256').update(data).digest('hex')
      console.log('hash1', hash1)
      console.log('hash2', hash2)
      data = data.slice(0, data.length - 64 - 8);
    }
